...
3. The real^[`r cpi_baseline_year` dollars] median home price over time.
4. The real median household income over time.

The first two metrics are the most important and incorporate the bottom two. However, the bottom two can give us better context in and of themselves. 
...
: Comparison of housing features over time {#tbl-housing-features}
...
There are three periods worth focusing on in @fig-real-household-income: Mid 1948 - late 1964, late 1964 - early 2012, and early 2012 - today. Let's examine each of these in turn.
...
This period saw significant growth over a brief period of time. From June 1948 to December 1964, real median household income grew from `r dollar(p1_ri_1)` to `r dollar(p1_ri_2)` representing an annual growth of `r round((((p1_ri_2/p1_ri_1) - 1)/getDif(as.Date("1948-06-01"), as.Date("1964-12-01")) * 100), 2)` pp per year. 

##### Late 1964 - early 2012

From December 1964 to January 2012, the real median household income only grew to `r dollar(p2_ri_2)` representing an annual growth of `r round((((p2_ri_2/p1_ri_2) - 1)/getDif(as.Date("1964-12-01"), as.Date("2012-01-01")) * 100), 2)` pp per year. Of course there were highs and lows over this period^[The peak income over this period was reached on July 1999 at `r dollar(p2_ri_rng$income_real[1])` representing a growth of `r round((((p2_ri_rng$income_real[1]/p1_ri_2) - 1)/getDif(as.Date("1964-12-01"), as.Date("1999-07-01")) * 100), 2)` pp per year. The lowest income over this period was reached on June 1982 at `r dollar(p2_ri_rng$income_real[2])` representing a decline of `r round((((p2_ri_rng$income_real[2]/p1_ri_2) - 1)/getDif(as.Date("1964-12-01"), as.Date("1999-07-01")) * 100), 2)` pp per year.], but overall growth was stagnant^[This becomes nebulous quickly. [Analysis by the Urban Institute](https://www.urban.org/sites/default/files/publication/65351/2000331-Beyond-the-Wage-Stagnation-Story.pdf) (a left-leaning think tank) have found that when you include all payments (e.g. benefits from employer), from 1979 - 2013 real income grew by roughly 1.1 pp per year. Furthermore, when you break up the analysis by different demographics (age, gender, education, etc.) you see some groups had significant growth (60+, women, higher education) while others lagged behind (25-34, men, less than BA).]. 
...
#| fig-cap: "Price-to-income ratio over time. Monthly forecasts with 3$\\sigma$ prediction interval. Linear model with 3$\\sigma$ confidence interval. Hover over the lines to see the percentile of the observation and ranges for forecasts."
#| column: body
#| out-width: "100%"
...
#| fig-cap: "Debt-to-income ratio over time. Monthly forecasts with 3$\\sigma$ prediction interval. Linear model with 3$\\sigma$ confidence interval. Hover over the lines to see the percentile of the observation and ranges for forecasts."
#| column: body
#| out-width: "100%"
